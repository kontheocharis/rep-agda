...
data Ty where
  ...
  Id   : {A : Ty Γ} → (a : Tm Γ A) → (b : Tm Γ A) → Ty Γ
...
data Tm where
  ...
  J   : {A : Ty Γ} → {u : Tm Γ A} → (P : Ty (Γ , A , Id (u [ p ]) q))
        → Tm Γ (P [ id , u [ id ] , {!   !} ])
        → ∀ {v} → (e : Tm Γ (Id u v)) → Tm Γ (P [ id , v [ id ] , {!   !} ])
...
data Sub~ where
  ...
  -- These are easier to work with as constructors rather than definitions
  wk-def : Sub~ rfl rfl (wk {A = A} σ) (σ ∘ p , coe rfl (sym [∘]) q)
  <>-def : Sub~ rfl rfl (< t >) (id , coe rfl (sym [id]) t)
...
data Tm~ where
  ...
  snd[] : Tm~ rfl rfl (snd t [ σ ]) (coe rfl (sym (trs <>-swap-Ty (rfl [ < fst[] > ]))) (snd (coe rfl Σ[] (t [ σ ]))))
...
coh'-Ty  : ∀ {Γ₂ : Con~ Γ₁ Γ₀} {A : Ty Γ₁} → Ty~ (sym Γ₂) (coe Γ₂ A) A
coh'-Ty {Γ₂ = Γ₂} {A = A} = sym (coh Γ₂ A)

coh'-Tm  : ∀ {Γ₂ : Con~ Γ₁ Γ₀} {A₂ : Ty~ Γ₂ A₁ A₀} {t : Tm Γ₁ A₁} → Tm~ (sym Γ₂) (sym A₂) (coe Γ₂ A₂ t) t
coh'-Tm {Γ₂ = Γ₂} {A₂ = A₂} {t = t} = sym (coh Γ₂ A₂ t)
...
-- snd[]-rfl : Tm~ rfl <>-swap-Ty (snd {B = B} t [ σ ]) (snd {B = B [ wk σ ]} (t [ σ ]))
-- snd[]-rfl = {!   !} 